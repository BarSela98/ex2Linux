...
string Mission::list_fetched_stocks() {
    ...
    set<string> stocksSet = getSetOfAllTheFilesInDir();
...
string Mission::exportAndCreateDBStocksData() {
    set<string> stocksSet = removeDupWord(list_fetched_stocks());
    if(stocksSet.size() == 0)
        return "";
...
set <string>  Mission::getSetOfAllTheFilesInDir(){
...
set<string> Mission::removeDupWord(string str)
{
