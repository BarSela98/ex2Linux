...
stockData::stockData(string stockName) {
    ...
    try {
        if (isFileExists(stockName + ".csv")) {
            importDataFromCSV();
        } else if (isFileExists(stockName + ".stock") && isFileExists(stockName + ".esp")) {
            ...
            throw std::invalid_argument("The stock isn't available");
        }
    }
    catch (exception &ex){throw std::invalid_argument("The stock isn't available");}
}

bool stockData::isFileExists (const std::string& name) {
...
void stockData::importDataFromCSV() {
    fstream csvFile;
    string fileName = this->stockName + ".csv";
    ...
    while (!csvFile.eof()) {
        ...
        std::getline(lineStream, tmpWord, ',');
        std::getline(lineStream, year, ',');
        std::getline(lineStream, month, ',');
        std::getline(lineStream, openRate, ',');
        std::getline(lineStream, highRate, ',');
        std::getline(lineStream, lowRate, ',');
        std::getline(lineStream, closeRate, ',');
        std::getline(lineStream, volume, ',');
        std::getline(lineStream, reportedEPS, ',');

        dataNodeLst[j] = new stockNode(stoi(year),stoi(month),stof(openRate),stof(highRate),stof(lowRate),stof(closeRate),stoi(volume));
        string newDate = year + month;
        this->epsMap.insert(pair<string, float>(newDate, stof(reportedEPS)));
        j++;
    }
    dataSize = j;
    csvFile.close();
