        ...
        try {
            ...
            switch (choose) /// task
            {
                ...
                case 4: { /// create and save all csv file
                    result = m.exportAndCreateDBStocksData(availableStockList);
                    strcpy(workerResponse, &result[0]);
                    break;
                }
            }
            ...
            write(fd2[WRITE], ex.what(), readSize);
        }
    }
}
...
void sigint_handler(int signum) /// signal handler - do task 4 when we get ^C
{
    ...
    {
        m.exportAndCreateDBStocksData(availableStockList);
    }
...
void loadDB (){/// EXTRACT THE FILES FROM THE ZIP FILE
    ...
    if ((file = fopen(DB.DB_NAME, "r"))) {
        ...
        for (auto it = availableStockList.begin(); it !=availableStockList.end(); ++it)
            cout << *it << " ";
        cout <<endl;
